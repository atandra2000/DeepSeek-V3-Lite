...
3. **You add a third kernel and skip the `ValueError` cap test. What breaks?** Nothing on the laptop — but the `196`-node CPU suite no longer documents the kernel's boundary, and a future config change could silently route the kernel into a register-spill regime on GPU with no test noticing (§3.7).
